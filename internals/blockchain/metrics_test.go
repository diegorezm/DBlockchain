...
func BenchmarkMetric_HashRateByDifficulty(b *testing.B) {
	for d := uint32(1); d <= 6; d++ {
		bc := NewBlockchain("")
		bc.Difficulty = d
		...
		for i := 0; i < 10; i++ {
			start := time.Now()
			_, nonceCount := bc.AppendBlock()
			elapsed := time.Since(start)

			hashRate := CalcHashRate(nonceCount, elapsed)

			writeHashRateMetric("hashrate_metrics", "HashRate", i, d, hashRate)
			b.Logf(
				"Dificuldade: %d — HashRate: %.2f H/s — Tempo: %v — Nonces: %d",
				d, hashRate, elapsed, nonceCount,
			)
		}
	}
}

func writeHashRateMetric(fileName, name string, iteration int, diff uint32, hashRate float64) {
	f := fmt.Sprintf("%s.csv", fileName)
	file, err := os.OpenFile(f, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		panic(err)
	}
	...
	info, _ := file.Stat()
	if info.Size() == 0 {
		_ = writer.Write([]string{"metric_name", "iteration", "difficulty", "hash_rate_hs"})
	}
...
func CalcHashRate(nonceCount int, elapsed time.Duration) float64 {
	if elapsed.Seconds() == 0 {
		return 0
	}
	return float64(nonceCount) / elapsed.Seconds()
}
